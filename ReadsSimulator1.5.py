...
class Read:
	...
	self.scoreGenerator(subSequence)
        self.seq = array.array('c',subSequence)
    ...
    def writeToFile(self,readsFileHandle):
        if read1Len == 0 and read2Len == 0:
            ...
            readsFileHandle.readsF.write(''.join(self.seq) + '\n')
...
#    for i in range(seqLen - min(readLen,read1Len+read2Len)):
    for i in range(0,seqLen-readLen):
        if singleEndFlag:
            ...
            while(multiple > 0):
	            ...
	            subSeq = sequence[i:i+readLen]
                    idPrefixSuffix = idPrefix + str(multiple)
	            read = Read(i,subSeq,idPrefixSuffix,error)
	            read.addSNP(snp)
            ...
            while(multiple > 0):
	            ...
	            #dynamicGap = random.sample(range(min(readLen,seqLen - i)-read1Len-read2Len),1)[0]
                    dynamicGap = max(random.sample(range(200,300),1)[0],readLen-read1Len-read2Len)
