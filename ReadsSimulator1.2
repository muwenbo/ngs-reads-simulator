...
class Read:
    ...
    def addSNP(self, snp):
        for index in range(len(snp.snpBufferIndex)):
            p = random.uniform(0,1)
            if(singleEndFlag):
                if(snp.snpBufferIndex[index] >= self.startPosition and snp.snpBufferIndex[index] < self.startPosition + self.seqLen and p <= snp.snpFraction[index]):
                    tempSeq = list(self.seq)
                 ...
                 if(((snp.snpBufferIndex[index] >= self.startPosition and snp.snpBufferIndex[index] < self.startPosition + read1Len) or (snp.snpBufferIndex[index] >= self.startPosition + self.seqLen - read2Len and snp.snpBufferIndex[index] < self.startPosition + self.seqLen)) and p <= snp.snpFraction[index]):
                    tempSeq = list(self.seq)
                    tempSeq[snp.snpBufferIndex[index] - self.startPosition] = snp.snpAlleles[index] 
                    self.seq = ''.join(tempSeq)
                    snp.addedCount[index] = snp.addedCount[index] + 1
    ...
    for i in range(len(sequence)-readLen):
        if singleEndFlag:
            ...
            while(multiple > 0):
                p = random.uniform(0,1)
                if(p <= multiple):   
	            dynamicGap = random.sample(range(readLen-read1Len-read2Len),1)[0]
